...
#        $record_tag = $self->{record_tag} = $record_tag->parent->prev_sibling->first_child;
        $record_tag ||= $self->{record_tag} =
          $record_tag->parent->prev_sibling->first_child;
    }
    else {
        for my $a ( keys %$atts ) {
            $a =~ s".*/([^/]+)$"$1";
...
sub get_data {
    ...
    my $col_structure      = $self->{col_structure};
    undef $col_structure
      unless $col_structure->{col_names}
    ...
    if ($col_map) {
        ...
        if ( !$col_structure ) {
            $have_col_names++;
            $col_structure =
              build_column_names( $colZ, $root, $root_tag, $colZ );
...
sub rowhash_to_array {
    ...
    my $pretty_col_names = $col_structure->{pretty_cols}
      if $col_structure->{pretty_cols};
    my @newvals;
    my %visited;
...
sub export {
    ...
    #$self->{twig}->print;
    $self->{twig}->set_pretty_print( $flags->{pretty_print} )
